...
class WhatsappService
{
    ...
    public function __construct(ChatbotRepository $chatbotRepo)
    {
    ...
    public function selectAction($input): bool|string
    {
        ...
        $event = $data['event'] ? explode('.', $data['event']) : null;
...
//        https://socializerx.com/api/set_webhook?webhook_url=https://8c1c-200-59-184-228.ngrok-free.app/&enable=true&instance_id=662150A3AE407&access_token=65ac077ef0c0a
        if(trim($message) == 'dar_baja' || trim($message) == 'dar_alta'){
            ...
            $client = \DB::connection('mysql_two')->table('clients')->select('id')->where('phone', 'like', '%' . $phone_search . '%')
    ...
    public function messagesUpdate($data): bool|string
    {
        $data = $data['data'];
        $success = '';
    ...
    public function clearRemoteJid($remoteJid): string|null
    {
        ...
        return ((isset($phone) && trim($phone) !== '') && str_contains($phone, ":")) ? explode(":", $phone)[0] : $phone;
    }

    public function searchMessageType($message): array|bool{
        if ($message['imageMessage']) {
            return $this->message_type_reference['imageMessage'];
        } elseif ($message['documentMessage']) {
            return $this->message_type_reference['documentMessage'];
        } elseif ($message['audioMessage']) {
            return $this->message_type_reference['audioMessage'];
        } elseif ($message['videoMessage']) {
            return $this->message_type_reference['videoMessage'];
        } elseif ($message['conversation']) {
            return $this->message_type_reference['conversation'];
        } else {
            return $this->message_type_reference['otherMessage'];
        }
    }
}
